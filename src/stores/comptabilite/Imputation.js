...
export const useImputationStore = defineStore("imputation", {
  ...
  actions: {
    ...
    async updateEcriture(ecriture) {
      ...
      if (data.module == 0) {
        this.ecritures.divers = this.ecritures.divers.map((e) => (e.id == data.id ? data : e));
      }
      return data;
    },
    async removeEcriture(ecritureId) {
      const data = await ImputationService.removeEcriture(ecritureId);
      this.ecritures.divers = this.ecritures.divers.filter((e) => e.id != ecritureId);
      this.ecritures.amendes = this.ecritures.amendes.filter((e) => e.id != ecritureId);
      this.ecritures.annuels = this.ecritures.annuels.filter((e) => e.id != ecritureId);
      return data;
    },
    ...
    async fetchEcrituresAnnuels(exerciceComptableId) {
      const data =
        await ImputationService.getEcrituresAnnuelsForExerciceComptable(exerciceComptableId);
      this.ecritures.annuels = [...data];
      return data;
    },
    async fetchEcrituresDivers(exerciceComptableId) {
      const data =
        await ImputationService.getEcrituresDiversForExerciceComptable(exerciceComptableId);
      this.ecritures.divers = data;
      return data;
    },
    async fetchAmendesExerciceComptable(exerciceComptableId) {
      const data = await ImputationService.getAmendesForExerciceComptable(exerciceComptableId);
    ...
    async imputerExercice(payload) {
      const data = await ImputationService.imputerExercice(payload.exercice_id, payload);
    ...
    async imputerIntervention(payload) {
      const data = await ImputationService.imputerIntervention(payload.intervention_id, payload);
      // Note: Cette action met à jour le store intervention, pas imputation
      return data;
    },
    async annulerImputationIntervention(interventionId) {
      const data = await ImputationService.annulerImputationIntervention(interventionId);
      // Note: Cette action met à jour le store intervention, pas imputation
      return data;
    },
    ...
    async addFraisIndemniteAnnuel(frais) {
      ...
      this.fraisIndemnites.annuels = this.fraisIndemnites.annuels.map((f) => {
        if (f.id === data.frais_indemnite_annuel_type_id) {
          return {
            ...f,
            frais_indemnite_annuels: [...f.frais_indemnite_annuels, data],
          };
        } else {
          return f;
        }
        ...
        if (f.id === data.frais_indemnite_annuel_type_id) {
          return {
            ...
            frais_indemnite_annuels: [
              ...f.frais_indemnite_annuels.filter((e) => e.id != data.id),
              data,
            ],
          };
        } else {
    ...
    async removeFraisIndemniteAnnuel(frais) {
      ...
      this.fraisIndemnites.annuels = this.fraisIndemnites.annuels.map((f) => {
        return {
          ...f,
          frais_indemnite_annuels: f.frais_indemnite_annuels.filter((e) => e.id != frais),
        };
      });
      return data;
    },
    ...
    async removeFraisIndemniteAnnuelType(frais) {
      ...
      this.fraisIndemnites.annuels = this.fraisIndemnites.annuels.filter((m) => m.id != frais);
      return data;
    },
    ...
    async removeIndemniteCours(indemniteId) {
      ...
      this.fraisIndemnites.cours = this.fraisIndemnites.cours.filter((m) => m.id != indemniteId);
      return data;
    },
    ...
    async removeIndemniteIntervention(indemniteId) {
      ...
      this.fraisIndemnites.interventions = this.fraisIndemnites.interventions.filter(
        (m) => m.id != indemniteId,
      );
      return data;
    },
