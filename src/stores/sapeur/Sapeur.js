...
export const useSapeurStore = defineStore("sapeur", {
  ...
  actions: {
    ...
    async saveActiveSapeur(payload) {
      const data = await SapeurService.saveSapeur(this.active.data.id, payload || this.active.data);
      this.active.data = data;
      this.liste = this.liste.map((s) =>
        s.id == data.id
          ? {
    ...
    async editTelephoneSapeur(telephone) {
      ...
      this.active.telephones = [...this.active.telephones.filter((p) => p.id !== data.id), data];
      return data;
    },
    async removeTelephoneSapeur(telephoneId) {
      const data = await SapeurService.removeTelephone(this.active.id, telephoneId);
      this.active.telephones = this.active.telephones.filter((p) => p.id !== telephoneId);
      return data;
    },
    // Permis
    ...
    async editPermis(payload) {
      ...
      this.active.permis = [...this.active.permis.filter((p) => p.id !== data.id), data];
      return data;
    },
    ...
    async removeSapeurFonction(fonctionId) {
      const data = await SapeurService.removeFonction(this.active.id, fonctionId);
      this.active.fonctions = this.active.fonctions.filter((p) => p.id !== fonctionId);
      this.active.data.fonction_id = data.main_fonction_id;
      return data;
    },
    // Grades
    ...
    async removeSapeurGrade(gradeId) {
      const data = await SapeurService.removeGrade(this.active.data.id, gradeId);
    ...
    async editSapeurCours(payload) {
      ...
      this.active.cours = [...this.active.cours.filter((c) => c.id !== data.id), data];
      return data;
    },
    async removeSapeurCours(coursId) {
      const data = await SapeurService.removeCours(this.active.data.id, coursId);
      this.active.cours = this.active.cours.filter((c) => c.id !== coursId);
      return data;
    },
    // Mutations
    async addMutation(payload) {
      const data = await SapeurService.addMutation(this.active.data.id, payload);
    ...
    async editMutation(payload) {
      const data = await SapeurService.editMutation(this.active.data.id, payload);
    ...
    async removeMutation(mutationId) {
      const data = await SapeurService.removeMutation(this.active.data.id, mutationId);
      this.active.mutations = this.active.mutations.filter((c) => c.id !== mutationId);
      this.liste = this.liste.map((s) => {
    ...
    async finFonctions(payload) {
      const data = await SapeurService.finFonctions(this.active.data.id, payload.fin, payload.ids);
      this.active.fonctions = data;
      return data;
    },
    async quitterGroupes(payload) {
      const data = await SapeurService.quitterGroupes(this.active.data.id, payload);
      // payload = identifiants d'appartenance quittés : on retire ces
      // appartenances de la liste, on ne l'écrase pas avec les ids.
      this.active.groupes = this.active.groupes.filter((g) => !payload.includes(g.id));
      return data;
    },
    async supprimerConvocation(payload) {
      return await SapeurService.supprimerConvocation(this.active.data.id, payload);
    },
    // Matériel
    ...
    removeMatPerso(ids) {
      ...
      this.active.materiels = this.active.materiels.filter((m) => !idsSet.has(m.id));
    },
