...
export const useExerciceStore = defineStore("exercice", {
  ...
  actions: {
    ...
    async removeSapeurs(sapeurIds) {
      ...
      this.active.sapeurs = this.active.sapeurs.filter((p) => !sapeurIds.includes(p.sapeur_id));
      this.liste = this.liste.map((e) =>
    ...
    async addHeure(heure) {
      ...
      if (this.active.id == data.exercice_id) {
        this.active.sapeurs = this.active.sapeurs.map((s) =>
          s.sapeur_id != data.sapeur_id ? s : { ...s, heures: [...(s.heures ?? []), data] },
        );
      }
      return data;
    },
      ...
      if (this.active.id == data.exercice_id) {
        this.active.sapeurs = this.active.sapeurs.map((s) =>
          s.sapeur_id != data.sapeur_id
            ...
            : {
                ...s,
                heures: (s.heures ?? []).map((h) => (h.id == data.id ? data : h)),
              },
        );
      }
      return data;
    ...
    async editPresenceExercice(presenceId, presence) {
      const { statut, sapeur } = await ExerciceService.editPresence(presenceId, presence);
      ...
      this.liste = this.liste.map((e) => (e.id == sapeur.exercice_id ? { ...e, statut } : e));
      if (this.active.id == sapeur.exercice_id) {
    ...
    async removeExcuse(exerciceId, sapeurId) {
      ...
      if (this.active.id == data.exercice_id) {
        this.active.sapeurs = this.active.sapeurs.map((s) => (s.id == data.id ? data : s));
      }
